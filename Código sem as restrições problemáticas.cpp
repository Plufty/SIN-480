...
int main(int argc, char* argv[])
{
	...
	int* p1;	//tempo de processamento por j1. Na prática é o somatório por produto  pertencente a A do tempo de descarregamento
	...
	int* p2;	//tempo de processamento por j2. Na prática é o somatório por produto  pertencente a B do tempo de carregamento

	char** nomeProduto; //nome do produto
	int* pd;		//Tempo de descarregamento por produto
	int* pc;		//Tempo de carregamento por produto	
	...
	int ht = 0;		// Horizonte de tempo, na prática é o somatório por produto de tempo de carregamento+tempo de descarregamento
	...
	int k;		//capacidade dos caminhões de saída
...
//	static const char* teste = R"(C:/Users/Plufty/Desktop/UFV/6º Período/SIN 480 - Pesquisa Operacional/Projeto/Código/teste.txt)";
	...
	//por T Definidos após a leitura de dados pois são dependentes deles
	...
	// Conjunto T será definido após a leitura de dados pois este depepnde dos dados de entrada
	...
	for (int p = 0; p < P; p++)
	{
		...
		fscanf(fp, "%d", &pd[p]); //Lendo tempo de descarregamento
		fscanf(fp, "%d", &pc[p]); //Lendo tempo de carregamento
	...
	T = ht; // o maior tempo é definido pelo horizonte de tempo
	tempo = new int[T]; // Inicializando neste local pois depende do valor do horizonte de tempo
	for (int i = 0; i < ht; i++)
	{
		tempo[i] = i; //o conjunto discreto de tempo é definido pelos tempo de 0 ao horizonte de tempo
	}
	for (int j1 = 0; j1 < J1; j1++)
	{
		for (int p = 1; p < sizeof(B); p++)
		{
			p1[j1] += pd[p]*nA[j1][p];//calculando p1
		}
	}
	for (int j2 = 0; j2 < J2; j2++)
	{
		for (int p = 1; p < sizeof(B); p++)
		{
			p2[j2] += pc[p]*nB[j2][p];//calculando p1
		}
	}
	fscanf(fp, "%d", &m1);
	fscanf(fp, "%d", &m2);
	...
	printf("Capacidade dos caminhoes de saida: %d\n", k);
